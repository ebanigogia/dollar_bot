...
def post_operation_selection(message, bot):
    try:
        ...
        if op not in options.values():
            ...
            raise Exception("Sorry there's no such operation \"{}\"!".format(op))
...
def post_overall_amount_input(message, bot):
    try:
        ...
        bot.send_message(chat_id, 'Income has been Updated!')
        return user_list
    except Exception as e:
        helper.throw_exception(e, message, bot, logging)   
...
def delete_income(message, bot):
    ...
    if str(chat_id) in user_list:
        ...
        helper.write_json(user_list)
    bot.send_message(chat_id, 'Income has been deleted!')

def view_income(message, bot):
    chat_id = message.chat.id
