...
bot=telebot.TeleBot('6574262829:AAH8bG0oOSabC2nBp0jP0_Lbp2rG7Moli58')
...
def deletion(message,bot):
    ...
    if lowercase_user_input in lowercase_categories:
        ...
        bot.register_next_step_handler(message, confirm_deletion,category_to_remove)
    else:
        bot.send_message(chat_id, f"'{category_to_remove}' Category is not in the list.")

def confirm_deletion(message,category_to_remove):
    chat_id = message.chat.id
